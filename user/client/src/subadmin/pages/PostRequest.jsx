...
const PostRequests = () => {
  ...
  console.log('🎯 PostRequests - targetAdminId:', targetAdminId);
  ...
  const { data: responseData, isLoading } = useQuery({
    queryKey: ["pendingPosts", currentPage, itemsPerPage, targetAdminId],
    queryFn: async () => {
      try {
        ...
        if (targetAdminId) {
          console.log('🎯 PostRequests - Adding targetAdminId to API call:', targetAdminId);
          params.append('adminId', targetAdminId);
        } else {
          console.log('⚠️ PostRequests - No targetAdminId found, using current user');
        }
        
        console.log('🌐 PostRequests - Final API URL params:', params.toString());
        
        const res = await axiosInstance.get(`/posts/admin/pending?${params.toString()}`);
        return res.data;
      } catch (err) {
  ...
  const updatePostStatus = useMutation({
    mutationFn: async ({ postId, status }) => {
      ...
      const url = `/posts/admin/${postId}/status${queryString ? `?${queryString}` : ''}`;
      
      console.log(`🎯 PostRequests - ${status} post ${postId} on behalf of admin:`, targetAdminId);
