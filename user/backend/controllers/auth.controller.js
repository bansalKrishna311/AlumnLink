...
export const signup = async (req, res) => {
	try {
		...
		// Optionally, send a notification to an admin for approval
		// await sendApprovalRequestEmail(adminEmail, user);
...
export const login = async (req, res) => {
	try {
		...
		if (!user.isApproved) {
			return res
				.status(403)
				.json({ message: "Account is awaiting admin approval. Please try again later." });
		}
